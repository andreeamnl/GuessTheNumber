...
def create_app():
    ...
    service_name = os.getenv('SERVICE_NAME', 'accounts_service')  # Fetch service name, like 'game_service_1'
    service_address = os.getenv('HOSTNAME', 'localhost')  # Docker container hostname
    service_port = int(os.getenv('SERVICE_PORT', '5001'))  # Dynamic port based on container (5002 is default)
...
if __name__ == "__main__":
    ...
    app = create_app()
    service_port = int(os.getenv('SERVICE_PORT', 5001))  # Default to 5002 if not specified
